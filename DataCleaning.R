...
# check how many tags have detections with runLen = 2
df.alltags %>%
...
df.alltags.path <- fun.getpath(filter(df.alltags.sub, 
                                      motusTagID %in% df.ambigTags$motusTagID, tagProjID == 
                                        proj.num)) %>% # create a boolean variable for ambiguous detections:
...
p + geom_point() + geom_path() + theme_bw() + 
  facet_wrap(~newID,scales = "free", ncol = 2) + theme(axis.text.x = element_text(angle = 45, 
                                                                                                                       vjust = 1, hjust = 1))
# Let's start with a very simple ambig tag: -337
...
# then plot th edetections by signal strength
